...
function textFromTextNode(node) {
  // Prefer decoded `.text` over `.rawText`. Raw preserves entities like `&amp;`;
  // re-escaping that produced `&amp;amp;` in output (visible as "&amp;" on screen).
  if (node.text != null) return node.text;
  if (node.rawText != null) return node.rawText;
  return "";
}

/** Fix bodyHtml saved before rawText→text fix (iterative `&amp;amp;` → `&amp;`). */
export function repairLegacyAmpDoubling(html) {
...
export function stripInsignificantBreakEdges(inner) {
  let s = String(inner || "");
  let prev;
  do {
    prev = s;
...
export function serializeMailerLiteNode(node) {
  ...
  if (tag === "a") {
    ...
    const inner = stripInsignificantBreakEdges(serializeMailerLiteChildren(node));
  ...
  if (tag === "p") {
    return `<p>${stripInsignificantBreakEdges(inner)}</p>`;
  }
  if (tag === "strong" || tag === "b" || tag === "em" || tag === "i" || tag === "u" || tag === "span") {
    return `<${tag}>${stripInsignificantBreakEdges(inner)}</${tag}>`;
  }
  return `<${tag}>${inner}</${tag}>`;
}
