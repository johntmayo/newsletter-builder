...
function stripInsignificantBreakEdges(inner) {
...
function splitParagraphInnerOnHardBreaks(inner) {
  const parts = [];
  const hardBreak = /(?:\s*<br\s*\/?>\s*){2,}/gi;
  let cursor = 0;
  let match;

  while ((match = hardBreak.exec(inner)) !== null) {
    parts.push(inner.slice(cursor, match.index).trim());
...
function expandHardBreakParagraphBlock(block) {
  ...
  const parts = splitParagraphInnerOnHardBreaks(inner);
