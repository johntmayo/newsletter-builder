...
function itemFromParagraph(el, id) {
  ...
  const inner = serializeParagraphInner(el);
  const bodyHtml =
...
function splitParagraphInnerOnHardBreaks(inner) {
  ...
  return parts.filter((part) => part && !isInsignificantParagraphInner(part));
}
...
function itemFromMerged(ps, ulEl, id) {
  ...
  let bodyHtml = "";
  for (const p of ps) {
    const inner = serializeParagraphInner(p);
    if (inner.trim() && !isInsignificantParagraphInner(inner)) {
      bodyHtml += `<p>${inner}</p>`;
    }
  }
  if (ulEl) {
    bodyHtml += serializeMailerLiteNode(ulEl);
  }
  return {
    id,
    type: "text",
